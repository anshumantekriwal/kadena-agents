...
const TradingAgentLauncher = () => {
  ...
  const handleAIRating = async () => {
    ...
    try {
      ...
      setReviewEnabled(rating > 7); // Only enable review if rating > 7
      if (rating > 7) {
    ...
    try {
      ...
      if (aiCode && interval) {
        ...
        try {
          ...
          const editCodeRes = await fetch(
            "https://baseline-knuv.onrender.com/edit-code",
            {
              method: "POST",
              headers: {
        ...
        try {
          ...
          const vercelRes = await fetch(
            "https://baseline-knuv.onrender.com/deploy-vercel",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (!vercelRes.ok) {
            ...
            if (
              vercelData.deploymentDetails &&
              vercelData.deploymentDetails.url
            ) {
              vercelDeploymentUrl = vercelData.deploymentDetails.url;
            } else if (
              vercelData.deploymentDetails &&
              vercelData.deploymentDetails.alias &&
              vercelData.deploymentDetails.alias.length > 0
            ) {
              vercelDeploymentUrl = `https://${vercelData.deploymentDetails.alias[0]}`; // Prepend https if not present
            } else {
              console.warn(
                "Could not determine Vercel deployment URL from response."
              );
            }
        ...
        if (vercelDeploymentUrl) {
          ...
          console.warn(
            "Skipping /trigger-aeod API call because Vercel deployment URL was not available."
          );
        }
      } else {
        console.warn(
          "AI code or interval missing, skipping new API call sequence."
        );
      }
      handleNext();
    } catch (error) {
                  ...
                  <div style={{ width: "90%" }}>
                    ...
                    <button
                      ...
                      style={{
                        width: "100%",
                        backgroundColor:
                          !agentBehavior.trim() || isGeneratingQuestions
                            ? "#666"
                            : "white",
                        color: "black",
                        padding: "12px",
                    ...
                    <button
                      ...
                      style={{
                        width: "100%",
                        backgroundColor:
                          !reviewEnabled || isFetchingAICode ? "#666" : "#222",
                        color: "white",
                        padding: "12px",
                        borderRadius: "8px",
                        border: "none",
                        cursor:
                          !reviewEnabled || isFetchingAICode
                            ? "default"
                            : "pointer",
                        fontWeight: "500",
                        marginBottom: "20px",
                    ...
                    {aiRating !== null && (
                      ...
                      >
                        ...
                        <span
                          style={{
                            color: aiRating >= 8 ? "#4caf50" : "#ff9800",
                          }}
                        >
                          {aiRating} / 10
                        </span>
                  ...
                  <div style={{ width: "90%" }}>
                    ...
                    >
                      ...
                      <div style={{ marginBottom: "20px" }}>
                        ...
                        <p style={{ margin: 0 }}>
                          {agentBehavior || "No behavior specified"}
                        </p>
                      </div>
                    </div>
