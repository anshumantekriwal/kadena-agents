...
async function baselineFunction() {
  try {
    ...
    // Swap 0.01 zUSD (n_b742b4e9c600892af545afb408326e82a6c0c6ed.zUSD) using KDA every invocation
    const account = "k:" + keyPair.publicKey;
    const tokenInAddress = "coin";
    const tokenOutAddress = "n_b742b4e9c600892af545afb408326e82a6c0c6ed.zUSD";
    const amountOut = "0.01";
    const slippage = 0.01; // 1% slippage tolerance

    let resultSwap = await swap({
      tokenInAddress,
      tokenOutAddress,
      account,
      chainId,
...
// // Schedule baselineFunction() to run every 10 minutes
setInterval(async () => {
  const now = new Date().toISOString();
  console.log(`Checking trigger at ${now}. Running baselineFunction every 10 minutes.`);
  await baselineFunction();
}, TEN_MINUTES);
